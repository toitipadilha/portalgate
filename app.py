...
def portaria():
    ...
    registros = conn.execute(
        f"SELECT v.*, p.nome as posto_nome FROM visitantes v LEFT JOIN postos p ON p.id=v.posto_id {e_filter}{p_filter} ORDER BY v.entrada DESC LIMIT 100",
        vals
    ).fetchall()
    conn.close()
...
def frota():
    ...
    motoristas = conn.execute(f"SELECT * FROM vigilantes WHERE habilitado_frota=1 AND ativo=1 {'AND empresa_id=?' if eid else ''}", vals).fetchall()
    registros = conn.execute(f"""
        SELECT rf.*, v.placa, v.modelo, vig.nome as motorista
        FROM registros_frota rf
        JOIN veiculos v ON v.id=rf.veiculo_id
        JOIN vigilantes vig ON vig.id=rf.motorista_id
        {'WHERE rf.empresa_id=?' if eid else ''}
...
def seguranca():
    ...
    p_f = "AND posto_id=?" if pid else ""
    ep_vals = tuple(v for v in [eid,pid] if v)
    e_vals = (eid,) if eid else ()

    postos = conn.execute("SELECT * FROM postos WHERE empresa_id=? AND ativo=1",(eid,)).fetchall() if eid else []
    vigilantes = conn.execute(f"SELECT vig.*, p.nome as posto_nome FROM vigilantes vig LEFT JOIN postos p ON p.id=vig.posto_id WHERE vig.ativo=1 {e_f} {p_f} ORDER BY vig.nome", ep_vals).fetchall()
    aso_vencidos = conn.execute(f"SELECT * FROM vigilantes WHERE aso_validade < ? AND ativo=1 {e_f}", (hoje,*e_vals)).fetchall()
    aso_vencendo = conn.execute(f"SELECT * FROM vigilantes WHERE aso_validade BETWEEN ? AND ? AND ativo=1 {e_f}", (hoje,hoje_30,*e_vals)).fetchall()
    porte_vencendo = conn.execute(f"SELECT * FROM vigilantes WHERE porte_arma=1 AND porte_validade <= ? AND ativo=1 {e_f}", (hoje_60,*e_vals)).fetchall()
    cnh_vencendo = conn.execute(f"SELECT * FROM vigilantes WHERE cnh_validade <= ? AND habilitado_frota=1 AND ativo=1 {e_f}", (hoje_60,*e_vals)).fetchall()
